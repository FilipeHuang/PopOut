...
def id3(data, features, label="class", depth=0, max_depth=10):
    ...
    if len(set(classes)) == 1: return Node(label=classes[0])
    # Paragem 2: sem atributos disponíveis ou profundidade máxima → folha com classe maioritária
    if not features or depth >= max_depth: return Node(label=majority)
    ...
    for val in values:
        ...
        if not subset: node.children[val] = Node(label=majority) # Subconjunto vazio → folha com classe maioritária do pai
        else: node.children[val] = id3(subset, remaining, label, depth + 1, max_depth)
    return node
...
def predict(node, row):
    ...
    if val not in node.children: return predict(list(node.children.values())[0], row)
...
def print_tree(node, indent=0, branch=""):
    ...
    prefix = "    " * indent
    if branch: print(prefix + f"[{branch}]")
    if node.is_leaf(): print(prefix + f"  - CLASS: {node.label}")
    else:
        print(prefix + f"  SPLIT ON: {node.feature}")
        for val, child in sorted(node.children.items()): print_tree(child, indent + 1, val)
